...
export function WizardStepCatalog({ data, onChange }: Props) {
  ...
  const { aiEnabled, aiLoading, aiError, setAiError, runBatchAi, runFieldAi } =
    useCatalogAi(data, onChange, ensureHtml);

  // ── Rule patch helper ──────────────────────────────────────────────────────

  const patchRule = useCallback(async (
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <select
          ...
          onChange={async (e) => {
            ...
            if (!v) return;
            await patchRule("list_release_url", { ...data.list_release_url, mode: "xpath", xpath: v });
          }}
        >
