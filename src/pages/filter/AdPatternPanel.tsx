...
export function AdPatternPanel({ patterns, onUpdate }: AdPatternPanelProps) {
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const newPatterns = text.split(/[\r\n]+/).map(l => l.trim()).filter(l => l && isValidRegex(l));
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs px-2 py-1 rounded-lg border transition-colors hover:opacity-80"
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs px-2 py-1 rounded-lg border transition-colors hover:opacity-80 disabled:opacity-40"
            style={{ borderColor: "var(--color-border)", color: "var(--color-text-muted)", background: "transparent" }}
          >
    ...
    >
      ...
      <div className="flex gap-2 mb-3">
        <div className="flex-1 relative">
          <input
            className={`w-full font-mono pr-8 ${inlineInputClass}`}
            style={{
              ...inlineInputStyle,
          ...
          {newPattern.trim() !== "" && (
            <span className="absolute right-2.5 top-1/2 -translate-y-1/2">
              {isValid
                ? <CheckCircle2 className="w-3.5 h-3.5" style={{ color: "var(--color-success, #22c55e)" }} />
          ...
          return (
            <div
              ...
              style={{
                ...
                borderColor: valid ? "var(--color-border)" : "color-mix(in srgb, var(--color-danger) 40%, transparent)",
              }}
            >
              {valid
                ? <CheckCircle2 className="w-3 h-3 shrink-0 opacity-40" style={{ color: "var(--color-success, #22c55e)" }} />
                : <AlertCircle className="w-3 h-3 shrink-0" style={{ color: "var(--color-danger)" }} />
              }
        ...
        {patterns.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 gap-2">
            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "var(--color-surface-2)" }}>
              <AlertCircle className="w-4 h-4" style={{ color: "var(--color-text-subtle)" }} />
            </div>
