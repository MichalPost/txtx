  ...
  return (
    <div className="flex gap-1.5">
      <input
        ...
        className="flex-1 rounded-[10px] border px-3 py-2 text-sm focus:outline-none"
        style={base}
        onFocus={(e) => {
...
export function ModelSelect({
  ...
  return (
    ...
    >
      {!options.includes(value) && value && <option value={value}>{value}（自定义）</option>}
      {options.map((m) => (
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-1.5">
        <textarea
          ...
          rows={2}
          className="flex-1 resize-none rounded-[10px] border px-3 py-1.5 text-xs focus:outline-none"
          style={{
            background: "var(--color-surface)",
            borderColor: "var(--color-border)",
            color: "var(--color-text)",
