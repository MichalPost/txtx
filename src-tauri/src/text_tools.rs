...
pub async fn merge_files(paths: Vec<String>, output: String) -> anyhow::Result<String> {
    ...
    let count = paths.len();
    for path in &paths {
        let decoded = crate::text_file::read_text_file_auto(path)
            .await
            .map_err(|e| anyhow::anyhow!("读取待合并文件失败 {}: {}", path, e))?;
...
pub async fn split_file(path: String, pattern: Option<String>) -> anyhow::Result<Vec<String>> {
    let decoded = crate::text_file::read_text_file_auto(&path)
        .await
        .map_err(|e| anyhow::anyhow!("读取待分割文件失败 {}: {}", path, e))?;
    let content = decoded.content;
