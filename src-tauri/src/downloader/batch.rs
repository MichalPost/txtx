...
pub(super) fn compute_target_date(cfg: &AppConfig) -> String {
    ...
    (now - chrono::Duration::days(actual_days))
        .format("%Y-%m-%d")
        .to_string()
}
...
pub(super) async fn execute_download_batch(
    ...
    let site_map: HashMap<String, WebsiteConfig> = config
        .websites
        .values()
        .map(|s| (s.domain_name.clone(), s.clone()))
        .collect();

    let novel_sem = Arc::new(Semaphore::new(normalize_worker_count(
        config.concurrency.novel_threads,
    )));
    let chapter_sem = Arc::new(Semaphore::new(normalize_worker_count(
        config.concurrency.chapter_threads,
    )));
    ...
    for candidate in candidates {
        ...
        tasks.push(tokio::spawn(async move {
            ...
            let result = download_novel(
                ...
                text_conv,
                ebook_conv,
                content_filter_cfg.clone(),
                rate_limit_cfg_clone.clone(),
            )
            .await;
