...
use super::logger::{log, FileLogger};
use crate::blacklist::Blacklist;
use crate::crawler::{fetch_novel_name, scan_site};
use crate::models::{
    AppConfig, BookCandidate, DownloadStats, ProgressEvent, ScanItem, WebsiteConfig,
};

async fn collect_existing_txt_names(base_dir: &Path) -> HashSet<String> {
...
pub async fn run_scan_and_filter(
    ...
    let enabled_sites: Vec<WebsiteConfig> = config
        .websites
        .values()
        .filter(|s| s.enabled)
        .cloned()
    ...
    log(
        ...
        logger,
        "info",
        format!("并发扫描 {} 个站点...", enabled_sites.len()),
    )
    .await;
    ...
    .await
    {
        Ok(candidates) => {
            log(
                ...
                logger,
                "info",
                format!("并发扫描完成，共 {} 条候选", candidates.len()),
            )
            .await;
        ...
        match scan_site(client, site_cfg, &config.network, target_date).await {
            Ok(mut candidates) => {
                ...
                log(
                    ...
                    logger,
                    "info",
                    format!("{}: 扫描到 {} 本", site_cfg.domain_name, count),
                )
                .await;
            ...
            Err(e) => {
                log(
                    ...
                    logger,
                    "error",
                    format!("{}: 扫描失败 - {}", site_cfg.domain_name, e),
                )
                .await;
            }
        }
...
async fn fill_missing_names(
    ...
    let fill_tasks: Vec<_> = candidates
        .iter()
        .filter(|c| c.name.is_empty())
        .map(|c| {
    ...
    let name_map: HashMap<String, String> = futures::future::join_all(fill_tasks)
        .await
        .into_iter()
        .filter_map(|r| r.ok())
...
async fn apply_filter(
    ...
    let existing_txt_names = collect_existing_txt_names(base_dir).await;
    ...
    for c in all_candidates {
        ...
        let exist_pri = priority_map
            .get(&entry.crawler_domain)
            .copied()
            .unwrap_or(999);
    ...
    let to_download: Vec<BookCandidate> = after_blacklist
        .into_iter()
        .filter(|c| {
            if existing_txt_names.contains(&c.name) {
                local_exists += 1;
                false
            } else {
                true
